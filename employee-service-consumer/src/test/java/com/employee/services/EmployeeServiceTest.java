...
class EmployeeServiceTest {
  ...
  void getAllEmployees() {
    ...
    org.springframework.data.domain.Sort orders =
            org.springframework.data.domain.Sort.by(org.springframework.data.domain.Sort.Direction.DESC, "dateOfEmployment");
    var pageReq = PageRequest.of(0, 10, orders);

    when(employeeRepository.findByStatus(EmployeeStatus.ACTIVE, pageReq)).thenReturn(new PageImpl<>(employees));
  ...
  void getEmployeeById() {
    ...
    when(employeeRepository.findByIdAndStatus(employeeId, EmployeeStatus.ACTIVE)).thenReturn(Optional.of(employee));

    assertThat(employeeService.getEmployee(employeeId)).isNotNull().usingRecursiveComparison().isEqualTo(employee);
  }
  ...
  void getEmployeeByIdWithInvalidIdThenNotFound() {
    ...
    var employeeId = "e26b1ed4-a8d0-11e9-a2a3-2a2ae2dbcce4";
    when(employeeRepository.findByIdAndStatus(employeeId, EmployeeStatus.ACTIVE)).thenReturn(Optional.empty());
  ...
  void updateEmployee() {
    ...
    when(employeeRepository.findByIdAndStatus(id, EmployeeStatus.ACTIVE)).thenReturn(Optional.of(createEmployeeEntity(id)));
    assertThat(employeeService.updateEmployee(id, req)).isNotNull().usingRecursiveComparison().isEqualTo(req);
  ...
  void deleteEmployee() {
    ...
    when(employeeRepository.findByIdAndStatus(id, EmployeeStatus.ACTIVE)).thenReturn(Optional.of(employee));
